...
using ReflectionInterface;  // Need the interface using statement and added to references so we can make use of it
using System;
...
namespace CrestronMastersMSS431InstructorProgram
{
    public class ControlSystem : CrestronControlSystem
    {
            ...
            if (args.SmartObjectArgs.ID == 3 && args.Sig.BoolValue == true)  //SmartObject ID 3 looking for presses only
            {
                switch (args.Sig.Name)
                {
                    ...
                    case "Tab Button 3 Press":
                        {
                            if (LoadPlugin("/user/Timslibrary.dll") == true)
                            {
                                myTp.BooleanInput[11].BoolValue = true;  // Enable for the transport
                                myTp.SmartObjects[3].BooleanInput["Tab Button 1 Select"].BoolValue = false;
                                myTp.SmartObjects[3].BooleanInput["Tab Button 2 Select"].BoolValue = false;
                                myTp.SmartObjects[3].BooleanInput["Tab Button 3 Select"].BoolValue = true;
                            }
                            else
                                myTp.BooleanInput[11].BoolValue = false;
                            break;
        ...
        private void MyTpSigChange(Crestron.SimplSharpPro.DeviceSupport.BasicTriList currentDevice, SigEventArgs args)
        {
            ...
            if (args.Sig.BoolValue == true)  // It's a press event
            {
                switch ((Buttons)args.Sig.Number) // we are using our enum for the join numbers so we have to cast it
                {
                    ...
                    case Buttons.checkbox1:
                        myTp.BooleanInput[12].BoolValue = !myTp.BooleanInput[12].BoolValue;
                        break;
                    case Buttons.checkbox2:
                        myTp.BooleanInput[13].BoolValue = !myTp.BooleanInput[13].BoolValue;
                        break;
                    case Buttons.checkbox3:
                        myTp.BooleanInput[14].BoolValue = !myTp.BooleanInput[14].BoolValue;
                        break;
        ...
        private void SaveSettings()
        {
            ...
            // Note the private strings below are for code clairity only
            ...
            // Lab1  Save the checkboxes.
            myConfig.Setting.Checkbox1 = myTp.BooleanInput[12].BoolValue;
            myConfig.Setting.Checkbox2 = myTp.BooleanInput[13].BoolValue;
            myConfig.Setting.Checkbox3 = myTp.BooleanInput[14].BoolValue;

            //Finally save the information
            myConfig.Save();
            
            myTp.BooleanInput[(uint)SubPages.ShowSettings].BoolValue = false; // hide the setins page
        ...
        private void UpdateSettingsPage()
        {
            ...
            myTp.BooleanInput[12].BoolValue = myConfig.Setting.Checkbox1;
            myTp.BooleanInput[13].BoolValue = myConfig.Setting.Checkbox2;
            myTp.BooleanInput[14].BoolValue = myConfig.Setting.Checkbox3;
        }
